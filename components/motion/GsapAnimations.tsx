...
export function GsapAnimations() {
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const toggle = root.querySelector<HTMLElement>("#billing-toggle");
      const lblMonthly = root.querySelector<HTMLElement>("#lbl-monthly");
      const lblAnnual = root.querySelector<HTMLElement>("#lbl-annual");
      if (toggle && lblMonthly && lblAnnual) {
        const setBilling = (annual: boolean) => {
          ...
          root.querySelectorAll<HTMLElement>(".price").forEach((p) => {
            const val = annual ? p.dataset.annual : p.dataset.monthly;
            if (!val) return;
            if (!prefersReduced) {
              const obj = { v: Number.parseFloat((p.textContent || "0").replace(/,/g, "")) || 0 };
              gsap.to(obj, {
                v: Number.parseFloat(val),
                duration: 0.5,
                ease: "power2.out",
                onUpdate: () => {
                  p.textContent = Math.round(obj.v).toString();
                },
              });
            } else {
              p.textContent = val;
          ...
          root.querySelectorAll<HTMLElement>(".period").forEach((s) => {
            s.textContent = annual ? "سنوياً" : "شهرياً";
          });
        };
        const onToggle = () => setBilling(!toggle.classList.contains("on"));
